...
const toAudio = (input, format = 'mp4') => {
    return new Promise((resolve, reject) => {
        const filename = path.join(__dirname, '../temp/' + Date.now() + '.' + format);
        const output = filename.replace(/\.(mp4|mkv)$/, '.mp3');

        fs.writeFileSync(filename, input);

        exec(`ffmpeg -i ${filename} -vn -ab 128k -ar 44100 -y ${output}`, (err) => {
